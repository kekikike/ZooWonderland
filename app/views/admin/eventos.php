<?php // app/Views/admin/eventos.php ?>
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        /* Estilos copiados y ajustados del ejemplo de animales */
        :root {
            --selva-dark: #2E7D32;
            --selva-med: #43A047;
            --selva-light: #66BB6A;
    ...
    <main>
        ...
        <div class="content-section">
            <div class="section-header">
                ...
                <a href="index.php?r=admin/eventos/crear" class="btn-primary"><i class="fas fa-plus-circle"></i> Nuevo Evento</a>
            </div>

            <!-- Filtros -->
           <form method="get" action="index.php" style="display: flex; gap: 1rem; margin-bottom: 2rem;">
                <input type="hidden" name="r" value="admin/eventos">
                ...
                <input type="date" name="fecha" value="<?= htmlspecialchars($filtros['fecha'] ?? '') ?>" style="padding: 0.8rem; flex:1; border-radius:6px; border:1px solid #ddd;">
                <input type="text" name="nombre" placeholder="Buscar por nombre..." value="<?= htmlspecialchars($filtros['nombre'] ?? '') ?>" style="padding: 0.8rem; flex:2; border-radius:6px; border:1px solid #ddd;">
              <select name="encargado_id" style="padding: 0.8rem; flex:1; border-radius:6px; border:1px solid #ddd;">
    <option value="">Todos los encargados</option>
    <?php foreach ($guias as $guia): ?>
        <option value="<?= $guia['id_guia'] ?>" <?= ($filtros['encargado_id'] ?? '') == $guia['id_guia'] ? 'selected' : '' ?>>
            <?= htmlspecialchars($guia['nombre_completo']) ?>
        </option>
    <?php endforeach; ?>
</select>
                <button type="submit" style="background: var(--selva-med); color: white; padding: 0.8rem 1.5rem; border-radius:6px; border:none; cursor:pointer;"><i class="fas fa-search"></i> Filtrar</button>
            </form>
                ...
                <table>
                   ...
                   <?php foreach ($eventos as $e): ?>
                    <tr>
                        ...
                        <td><?= date('d/m/Y H:i', strtotime($e['fecha_inicio'])) ?> - <?= date('d/m/Y H:i', strtotime($e['fecha_fin'])) ?></td>
                        <td><?= $e['tiene_costo'] ? 'Bs ' . number_format($e['precio'], 2) : 'Gratuito' ?></td>
                        <td><?= htmlspecialchars($e['encargado_nombre'] ?? 'No asignado') ?></td>
                        <td><?= htmlspecialchars($e['vigencia']) ?></td>
                        <td><?= $e['limite_participantes'] ?? 'Sin límite' ?></td>
                        <td class="actions">
                            <a href="index.php?r=admin/eventos/detalle&id=<?= $e['id_evento'] ?>" class="view"><i class="fas fa-eye"></i> Detalle</a>
                            <a href="index.php?r=admin/eventos/editar&id=<?= $e['id_evento'] ?>" class="edit"><i class="fas fa-pen"></i> Editar</a>
                            <a href="#" class="delete" onclick="showDeleteModal(<?= $e['id_evento'] ?>)"><i class="fas fa-trash"></i> Eliminar</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
                    </tbody>
            ...
            <div id="deleteModal" class="modal">
                <div class="modal-content">
                    ...
                    <form method="POST" action="index.php?r=admin/eventos/eliminar">
