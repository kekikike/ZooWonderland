...
$reportesGuias = $reportesGuias ?? collect();

$inicio = request('inicio', '');
$fin    = request('fin',    '');

// Totales reservas
$totalReservas    = 0; $totalCupos       = 0;
$totalPagadasR    = 0; $totalPendientesR = 0;
...
// Totales compras
$totalCompras     = 0; $totalIngresos    = 0.0;
$totalPagadasC    = 0.0; $totalPendientesC = 0.0;
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Open Sans', sans-serif; background: var(--gris-claro); color: #333; scroll-behavior: smooth; line-height: 1.6; }
        ...
        .header-content h1 { font-family: 'Montserrat', sans-serif; font-weight: 800; font-size: 1.6rem; display: flex; align-items: center; gap: 12px; }
        .back-link { background: rgba(255,255,255,0.15); padding: 8px 16px; border-radius: 8px; color: white; text-decoration: none; font-weight: 600; font-size: 0.9rem; transition: var(--transicion); }
        ...
        .nav-item { flex: 1; text-align: center; padding: 10px; text-decoration: none; color: var(--oscuro); font-family: 'Montserrat', sans-serif; font-weight: 700; font-size: 0.85rem; border-radius: 40px; transition: var(--transicion); }
        ...
        .kpi-card { background: var(--blanco); padding: 1.5rem; border-radius: 16px; box-shadow: var(--sombra); display: flex; align-items: center; gap: 15px; border-left: 5px solid var(--verde-selva); }
        ...
        .filter-card { background: var(--blanco); border-radius: 16px; padding: 1.5rem; box-shadow: var(--sombra); margin-bottom: 2.5rem; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); align-items: flex-end; gap: 1rem; }
        .filter-group label { display: block; font-size: 0.75rem; font-weight: 800; color: #666; margin-bottom: 6px; }
        .filter-group input { width: 100%; padding: 10px; border: 2px solid #eee; border-radius: 10px; font-family: inherit; }
        .btn-filtrar { background: var(--verde-selva); color: white; border: none; height: 45px; border-radius: 10px; font-weight: 700; cursor: pointer; transition: var(--transicion); }
        ...
        .table-wrap { background: var(--blanco); border-radius: 16px; overflow: hidden; box-shadow: var(--sombra); }
        ...
        .chip { padding: 4px 12px; border-radius: 20px; font-size: 0.75rem; font-weight: 700; }
        ...
        .btn-pdf { text-decoration: none; padding: 10px 20px; border-radius: 10px; font-weight: 700; font-size: 0.85rem; display: flex; align-items: center; gap: 8px; transition: var(--transicion); }
...
<div class="wrapper">
    ...
    <div class="pdf-row">
        <a href="/admin/reportes/pdf?tipo=reservas&inicio={{ $inicio }}&fin={{ $fin }}" class="btn-pdf btn-pdf-res"><i class="fa-solid fa-file-pdf"></i> Reservas</a>
        <a href="/admin/reportes/pdf?tipo=compras&inicio={{ $inicio }}&fin={{ $fin }}"  class="btn-pdf btn-pdf-com"><i class="fa-solid fa-file-pdf"></i> Compras</a>
        <a href="/admin/reportes/pdf?tipo=guias&inicio={{ $inicio }}&fin={{ $fin }}"    class="btn-pdf btn-pdf-gui"><i class="fa-solid fa-file-pdf"></i> Guías</a>
    </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @else
                                <span class="chip chip-orange">Inactivo</span>
                            @endif
                        </td>
                    </tr>
                    @endforeach
