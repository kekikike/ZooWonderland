...
<body>
    ...
    <main>
        ...
        <div class="content-section">
            @if(session('flash_mensaje'))
                <div class="flash {{ session('flash_tipo', 'ok') }}">
                    {{ session('flash_mensaje') }}
                </div>
            @endif

            <div class="section-header">
                <h2><i class="fas fa-compass"></i> Recorridos</h2>
                <a href="/admin/recorridos/crear" class="btn-primary"><i class="fas fa-plus-circle"></i> Nuevo Recorrido</a>
            </div>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $rec->tipo ?? '—' }}</td>
                            <td><strong>Bs. {{ number_format($rec->precio ?? 0, 2) }}</strong></td>
                            <td class="actions">
                                <a href="/admin/recorridos/editar?id={{ $rec->id_recorrido }}" class="edit" title="Editar">
                                    <i class="fas fa-pen"></i> Editar
                                </a>
                                <a href="/admin/recorridos/eliminar?id={{ $rec->id_recorrido }}" class="delete" title="Eliminar"
                                   onclick="return confirm('¿Confirma eliminar este recorrido? Esta acción no afectará reservas activas.')">
