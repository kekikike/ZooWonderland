    ...
    <main>
        ...
        <div class="content-section">
            <div class="section-header">
                ...
                <a href="/admin/eventos/editar?id={{ $evento->id_evento }}" class="btn-primary">
                    <i class="fas fa-pen"></i> Editar Evento
                </a>
            </div>

            <div class="detalle-section">
                <div class="detalle-item"><strong>Nombre:</strong> {{ $evento->nombre_evento }}</div>
                <div class="detalle-item"><strong>Descripción:</strong> {{ $evento->descripcion }}</div>
                <div class="detalle-item"><strong>Fecha inicio:</strong> {{ \Carbon\Carbon::parse($evento->fecha_inicio)->format('d/m/Y H:i') }}</div>
                <div class="detalle-item"><strong>Fecha fin:</strong> {{ \Carbon\Carbon::parse($evento->fecha_fin)->format('d/m/Y H:i') }}</div>
                <div class="detalle-item">
                    <strong>Costo:</strong>
                    {{ $evento->tiene_costo ? 'Bs ' . number_format($evento->precio, 2) : 'Gratuito' }}
                </div>
                <div class="detalle-item">
                    <strong>Encargado:</strong>
                    {{ $evento->guia?->usuario?->nombre1 ?? 'No asignado' }}
                    {{ $evento->guia?->usuario?->apellido1 ?? '' }}
                </div>
                <div class="detalle-item"><strong>Lugar:</strong> {{ $evento->lugar }}</div>
            ...
            <div class="detalle-section">
                ...
                @else
                    <table class="actividades-table">
                        ...
                        <tbody>
                            @foreach($evento->actividades as $act)
                            <tr>
                                <td>{{ $act->nombre_actividad }}</td>
                                <td>{{ $act->descripcion ?? 'No especificada' }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            <a href="/admin/eventos" class="btn-primary" style="display: block; text-align: center; margin-top: 2rem; width: fit-content; margin: 0 auto;">
