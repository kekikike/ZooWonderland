...
<head>
    ...
    <style>
        ...
        .alert-success { background: #e8f5e9; color: var(--selva-dark); border-left: 4px solid var(--selva-dark); }
        .alert-error   { background: #fee2e2; color: #c62828; border-left: 4px solid #c62828; }
        ...
        .btn-primary { background: linear-gradient(135deg, var(--selva-light), var(--jungle-gold)); color: white; padding: 0.9rem 2rem; border-radius: 25px; text-decoration: none; font-weight: 700; display: inline-flex; align-items: center; gap: 0.8rem; box-shadow: 0 4px 15px rgba(46,125,50,0.3); cursor: pointer; transition: var(--trans); border: none; }
        .btn-primary:hover { transform: translateY(-3px); }
        .filtros-form { display: flex; gap: 1rem; margin-bottom: 2rem; flex-wrap: wrap; }
        .filtros-form select,
        .filtros-form input { padding: 0.8rem; flex: 1; border-radius: 6px; border: 1px solid #ddd; font-family: inherit; min-width: 150px; }
        .btn-filtrar { background: var(--selva-med); color: white; padding: 0.8rem 1.5rem; border-radius: 6px; border: none; cursor: pointer; font-weight: 600; white-space: nowrap; }
        ...
        tbody tr:hover { background: rgba(255,193,7,0.05); }
        ...
        .view:hover   { background: rgba(2,136,209,0.25); }
        .edit   { color: var(--selva-light); background: rgba(46,125,50,0.1); }
        .edit:hover   { background: rgba(46,125,50,0.25); }
        .delete { color: #ff6f6f; background: rgba(255,111,111,0.1); }
        .delete:hover { background: rgba(255,111,111,0.25); }
        ...
        .modal-content { background: white; padding: 2rem; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.2); text-align: center; max-width: 400px; width: 90%; }
        .modal-btn { margin: 1rem 0.5rem; padding: 0.8rem 1.5rem; border: none; border-radius: 6px; cursor: pointer; font-size: 1rem; }
...
<main>
    ...
    <div class="content-section">
        <div class="section-header">
            ...
            <a href="/admin/eventos/crear" class="btn-primary">
                <i class="fas fa-plus-circle"></i> Nuevo Evento
            </a>
        </div>

        <form method="GET" action="/admin/eventos" class="filtros-form">
            <input type="text" name="nombre" placeholder="Buscar por nombre..."
                   value="{{ $filtros['nombre'] ?? '' }}">

            <input type="date" name="fecha_inicio" title="Desde"
                   value="{{ $filtros['fecha_inicio'] ?? '' }}">

            <input type="date" name="fecha_fin" title="Hasta"
            ...
            <select name="estado">
                ...
                <option value="pasado"    {{ ($filtros['estado'] ?? '') === 'pasado'    ? 'selected' : '' }}>Pasados</option>
            </select>
