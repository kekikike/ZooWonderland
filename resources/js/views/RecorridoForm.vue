  ...
  <div>
    ...
    <main>
      <div class="form-section">
        <h2><i class="fas fa-route"></i>
          <span v-if="isEditing">Editar</span><span v-else>Crear</span> Recorrido
        </h2>
        ...
        <form @submit.prevent="guardarRecorrido">
          ...
          <div class="form-group">
            ...
            <div v-else>
              <div v-for="area in areas" :key="area.id_area" class="checkbox-item">
                <input
                  type="checkbox"
                  ...
                  v-model="form.areas"
                >
                <label :for="'area-' + area.id_area">{{ area.nombre }}</label>
              </div>
            </div>
...
export default {
  props: {
    recorridoId: { type: Number, default: null }
  },
  data() {
    return {
      isEditing: !!this.recorridoId,
      form: {
        nombre: '',
        tipo: 'No Guiado',
        precio: 0,
  ...
  methods: {
    ...
    cargarRecorrido() {
      api
        .get(`/api/recorridos/${this.recorridoId}`)
        .then(response => {
    ...
    guardarRecorrido() {
      ...
      if (this.isEditing) datos.id = this.recorridoId;
...
.loading {
  ...
  border-top: 3px solid var(--selva-dark);
  border-radius: 50%;
